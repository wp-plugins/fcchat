...
jGo.fcchat_toolbar.custombutton = function(id,s){
    ...
    var create_menu_item = function(){
  	  return"<div class='fc_custombutton"+id+"_button' style='position: absolute;width:"+config.menu_item.width+"px;top:0px;left:0px;'>" +
  				//"<div class='fc_abbtn' >" +
  					"<a "+(config.menu_item.newtab?"target=_blank":"")+" title='"+config.menu_item.text+"' href=\""+(config.menu_item.href?config.menu_item.href:"javascript:jGo.fcchat_toolbar.custombutton"+id+".tgl_dialog()")+"\" onclick=\""+(config.menu_item.onclick||"")+"\">" +
  						(config.menu_item.icon!=''?"<img style='display:block;position:absolute;top:"+ (jGo.fcchat_toolbar.style.icons_16px.top+fc_chat.get_ie_offset()) + "px; left: "+config.menu_item.padding_left+"px;width:16px;height:16px;"+config.menu_item.icon_css+"' src='"+config.menu_item.icon+"'>":"") +
  						"<div id='jGo_custombutton"+id+"_txt_inner' style='position:absolute;top:0px;left:"+(config.menu_item.icon!=''?31:10)+"px;width:"+(config.menu_item.width-28)+"px;padding-top:"+(config.menu_item.padding_top+fc_chat.get_ie_offset())+"px;"+jGo.fcchat_toolbar.style.text+";"+config.menu_item.text_css+";'>" +config.menu_item.text+ "</div>" +
  					"</a>" +
  				//"</div>" +
  			"</div>";
  	};
  	var create_dialog = function(){
  		  dialog= "<div class='jGo_app jGo_myapp jGo_dialog' id='fc_custombutton"+id+"_dialog' style='position:absolute;display: none;visibility: visible;"+config.dialog_box.css+";text-align:left;height:"+config.dialog_box.height+"px;width:"+config.dialog_box.width+"px;top: 113px; left:210px; z-index:" + z_b + ";overflow:hidden;'>" +
  			...
  			"<div  id='fc_view_custombutton"+id+"_dialog' style='position:absolute;width:"+config.dialog_box.width+"px;"+config.dialog_box.info_box_css+";overflow:hidden'></div></div>";
  			return dialog;
  	};
  	var render_dialog_content = function(){
  		if(rendered==0){
  			var content="<iframe onload='this.style.visibility=\"visible\";' src='"+config.dialog_box.iframe_src+"' frameborder=0 style='visibility:hidden;width:"+config.dialog_box.width+"px;height:"+config.dialog_box.content_height+"px'>";
